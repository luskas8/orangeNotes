...
import levingNote from "@utils/leavingNote"
import { HTMLProps, MouseEvent, useState, useState } from "react"
...
export const NavItem = ({ itemLabel, icon, authorization, path, isExact }: NavItemProps) => {
    ...
    async async function handleItemClick(e: MouseEvent<HTMLButtonElement>) {
        if (itemLabel === "back") {
            if (usingLeavingPage) {
    ...
    return (
        <Box as="li" role="none" listStyleType="none" key={v4()} color={isActive ? "orange.600" : "gray.600"} boxSize={{ base: "24px", md: "32px" }} minWidth="0px" margin="0px !important">
            <Button isLoading={isLoading} isLoading={isLoading} role="menuitem" minWidth="0px" bg="transparent" padding={{ base: "0", md: "5px" }} onClick={handleItemClick} width="100%" height="100%">
